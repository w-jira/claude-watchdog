...
install_npm_dog_shim() {
  ...
  {
    printf '#!/bin/bash\n'
    printf '# Generated by claude-watchdog setup. Keep the npm package dog as the source of truth.\n'
    printf 'exec %q "$@"\n' "$target"
  } > "${HOME}/bin/dog"
...
if [ "$NPM_PACKAGE_INSTALL" = "1" ]; then
  install_npm_dog_shim
  log "installed ~/bin/dog shim to delegate to the npm package CLI"
else
