...
NPM_PACKAGE_INSTALL=0
case "$ROOT" in
  */node_modules/@wjira/claude-watchdog) NPM_PACKAGE_INSTALL=1 ;;
esac
INSTALL_DEPS=0
INSTALL_CLAUDE=0
START_SERVICE=0
...
install_npm_bin_shim() {
  local name="$1" target="$2"
  install -d -m 700 "${HOME}/bin"
  {
    printf '#!/bin/bash\n'
    printf '# Generated by claude-watchdog setup. Keep the npm package as the source of truth.\n'
    printf 'set -euo pipefail\n'
    printf 'name=%q\n' "$name"
    printf 'original_target=%q\n' "$target"
    cat <<'SH'
if command -v npm >/dev/null 2>&1; then
  npm_root="$(npm root -g 2>/dev/null || true)"
  candidate="${npm_root}/@wjira/claude-watchdog/bin/${name}"
  if [ -n "$npm_root" ] && [ -x "$candidate" ] && [ "$candidate" != "$0" ]; then
    exec "$candidate" "$@"
  fi
fi
exec "$original_target" "$@"
SH
  } > "${HOME}/bin/${name}"
  chmod 700 "${HOME}/bin/${name}"
}

install_npm_dog_shim() {
  local target="${ROOT}/bin/dog"
  install -d -m 700 "${HOME}/bin"
  {
    printf '#!/bin/bash\n'
    printf '# Generated by claude-watchdog setup. Keep the npm package as the source of truth.\n'
...
if command -v npm >/dev/null 2>&1; then
  npm_root="$(npm root -g 2>/dev/null || true)"
  candidate="${npm_root}/@wjira/claude-watchdog/bin/dog"
  if [ -n "$npm_root" ] && [ -x "$candidate" ] && [ "$candidate" != "$0" ]; then
    exec "$candidate" "$@"
  fi
fi
...
SH
  ...
  chmod 700 "${HOME}/bin/dog"
}

expose_npm_package_to_systemd() {
  [ "$NPM_PACKAGE_INSTALL" = "1" ] || return 0
  python3 - "$ROOT" "${SYSTEMD_DIR}/telegram-claude.service" "${SYSTEMD_DIR}/telegram-claude-watchdog.service" <<'PY'
import sys
from pathlib import Path

root = sys.argv[1]
if "\n" in root or not root.startswith("/"):
    raise SystemExit("invalid npm package root for systemd bind")
line = f"BindReadOnlyPaths={root}"
for unit_arg in sys.argv[2:]:
    unit = Path(unit_arg)
    text = unit.read_text(encoding="utf-8")
    if line in text:
        continue
    marker = "ProtectHome=tmpfs\n"
    if marker not in text:
        raise SystemExit(f"missing ProtectHome marker in {unit}")
    unit.write_text(text.replace(marker, marker + line + "\n", 1), encoding="utf-8")
PY
}
...
if [ "$NPM_PACKAGE_INSTALL" = "1" ]; then
  install_npm_dog_shim
  install_npm_bin_shim claude-tele "${ROOT}/bin/claude-tele"
  install_npm_bin_shim claude-tele-watchdog "${ROOT}/bin/claude-tele-watchdog"
  install_npm_bin_shim claude-tele-patch-telegram-plugin "${ROOT}/bin/claude-tele-patch-telegram-plugin"
  install_npm_bin_shim claude-tele-replay-missed "${ROOT}/bin/claude-tele-replay-missed"
  install_npm_bin_shim claude-tele-control-mcp.py "${ROOT}/bin/claude-tele-control-mcp.py"
  log "installed ~/bin shims to delegate to the npm package CLI"
else
