...
install_npm_bin_shim() {
  local name="$1" target="$2" package_name="${3:-$1}"
...
if command -v npm >/dev/null 2>&1; then
  ...
  candidate="${npm_root}/@wjira/claude-watchdog/bin/${package_name}"
...
if [ "$NPM_PACKAGE_INSTALL" = "1" ]; then
  install_npm_dog_shim
  install_npm_bin_shim claude-tele "${ROOT}/bin/claude-tele-macos" claude-tele-macos
