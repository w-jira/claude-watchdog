...
def test_npm_setup_installers_use_shim_instead_of_copying_stale_dog():
    for script_name in ("install.sh", "install-macos.sh"):
        ...
        assert "install_npm_dog_shim" in text
        assert "installed ~/bin/dog shim to delegate to the npm package CLI" in text
...
def test_npm_installed_setup_finds_packaged_installer_through_bin_symlink(tmp_path):
    ...
    assert "installed ~/bin/dog shim to delegate to the npm package CLI" in combined
    ...
    path_shadow_help = subprocess.run(
        ["dog", "help"],
        cwd=tmp_path,
        env={**os.environ, **env, "PATH": f"{home / 'bin'}:{env['PATH']}"},
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=True,
